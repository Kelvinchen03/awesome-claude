...
slug: heyclaude-mcp
category: mcp
description: Search the public HeyClaude registry, fetch entry details, inspect compatibility, and build issue-first submission URLs through MCP.
...
seoDescription: Use the official HeyClaude MCP server to search registry entries, fetch install guidance, inspect compatibility, and prepare issue-first content submissions.
...
repoUrl: "https://github.com/JSONbored/claudepro-directory/tree/main/packages/mcp"
documentationUrl: "https://github.com/JSONbored/claudepro-directory/tree/main/packages/mcp"
installable: true
...
Submission helpers validate drafts and generate prefilled HeyClaude/GitHub URLs
for a human-reviewed issue-first workflow.
...
- `get_submission_schema`: inspect category-specific submission fields and issue-template metadata.
...
- `build_submission_urls`: build prefilled HeyClaude submit and GitHub issue URLs.
...
```text
Validate this HeyClaude content submission draft and build a prefilled issue URL.
```
...
or GitHub issue URL, then let the maintainer-reviewed import workflow handle
