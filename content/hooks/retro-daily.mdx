...
scriptBody: |-
  ...
  additional_context = "\n".join([
      ...
      "Use it only as a summary of recent Claude Code activity and improvement opportunities.",
      "",
...
Retro Daily reads local Claude Code session history from `~/.claude/projects/*.jsonl` to compute metrics. It writes derived state, logs, scout results, and session tags under `~/.claude/metrics`. The startup debug log is created at `~/.claude/metrics/startup.log` with user-only file permissions when possible.
